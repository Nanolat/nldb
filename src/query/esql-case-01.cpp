...
void open_db(char * db_open_option )
{
	...
	{
		printf("The database open option should be \"master\", \"slave\", or \"slave-master\". ");
		exit(-1);
	}
}
...
void insert_into_dept()
{
    ...
    */
    {
        ...
        SQLCA.SQLCODE = nldb_table_put(tx, esql_table_dept, nldb_key, nldb_value);
    }
    CHECK_ERROR(SQLCA.SQLCODE);
    ...
    */
    ...
    dept dept_mkt = { {MKT_DEPT_NO}, {"Marketing", "Santa Clara, CA, U.S."} };
    /*
        ...
        if (rc == NLDB_OK)
        {
            ...
            }
            {
				...
				while(1) {
					...
					rc = nldb_cursor_move_forward(cursor_emp, &nldb_emp_key, &nldb_emp_value);
					if (rc == NLDB_ERROR_END_OF_ITERATION)
					{
						break;
					}
					emp::key_t * emp_key = (emp::key_t *) nldb_emp_key.data;
					emp::value_t * emp_value = (emp::value_t *) nldb_emp_value.data;
					tx_assert( emp_key );
					tx_assert( emp_value );
					...
					while(1) {
						...
						rc = nldb_cursor_seek_forward(cursor_dept, dept_min_key);
						if (rc != NLDB_OK)
						{
			            	SQLCA.SQLCODE = rc;
							goto close_cursor_dept;
						...
						rc = nldb_cursor_move_forward(cursor_dept, &nldb_dept_key, &nldb_dept_value);
						if (rc == NLDB_ERROR_END_OF_ITERATION)
						{
							break;
						}

						struct dept::key_t * dept_key = (dept::key_t *) nldb_dept_key.data;
						struct dept::value_t * dept_value = (dept::value_t *) nldb_dept_value.data;

						tx_assert( dept_key );
						tx_assert( dept_value );
						tx_assert( sizeof(*dept_key) == nldb_dept_key.length );
						tx_assert( sizeof(*dept_value) == nldb_dept_value.length );

						if ( emp_value->dept_no == dept_key->dept_no )
						{

							struct dept_emp dept_emp_record = { {dept_key->dept_no, emp_key->emp_no}, { 2010} };
							// The same code generated by EXEC SQL INSERT INTO dept VALUES :dept_emp_record ;
						    {
						        ...
						        nldb_key_t nldb_key = { & dept_emp_record.key, sizeof(dept_emp_record.key) };
						        nldb_value_t nldb_value = { & dept_emp_record.value, sizeof( dept_emp_record.value ) };
						        SQLCA.SQLCODE = nldb_table_put(tx, esql_table_dept, nldb_key, nldb_value);
						    }
						    if (SQLCA.SQLCODE != NLDB_OK)
						    {
...
close_cursor_dept:
            rc = nldb_cursor_close( cursor_dept );
            if (rc != NLDB_OK)
            	SQLCA.SQLCODE = rc;
        }
close_cursor_emp:
        rc = nldb_cursor_close( cursor_emp );
        if (rc != NLDB_OK)
        	SQLCA.SQLCODE = rc;
    }
	...
	// generated code for nested index join
    {
        ...
        if (rc == NLDB_OK)
        {
			...
			while(1) {
				...
				struct emp::value_t * emp_value = (emp::value_t *) nldb_emp_value.data;
				tx_assert( emp_key );
				tx_assert( emp_value );
				tx_assert( sizeof(*emp_key) == nldb_emp_key.length );
				tx_assert( sizeof(*emp_value) == nldb_emp_value.length );
				...
				// Get the matching dept
				{
					...
					nldb_value_t nldb_dept_value ;
					rc = nldb_table_get( tx, esql_table_dept, nldb_dept_key, &nldb_dept_value);
			...
			while(1) {
				...
				if ( strcmp(emp_value->name, "Kangmo Kim") != 0 ) {
...

					{
						...
						nldb_value_t nldb_emp_value = { emp_value, sizeof(*emp_value) };

						SQLCA.SQLCODE = nldb_table_put(tx, esql_table_emp, nldb_emp_key, nldb_emp_value );
	    ...
	    if (rc == NLDB_OK)
	    {
			...
			while(1) {
				...
				if ( strcmp(emp_value->name, old_hq_name) == 0 ) {
					...
					memcmp(emp_value->name, new_hq_name, sizeof(emp_value->name));
...
{
	printf("\nquery <option> \n");
	printf("Where option is create-database, drop-database, master, slave, or slave-master.\n\n");
	printf("1. create-database - Create a database, create tables in it. \n ");
	printf("2. drop-database   - Drop thables in the created database, drop the database. \n");
	printf("3. master - Open the database as the master. \n");
	printf("4. slave - Open the database creates a slave. \n");
	printf("5. slave-master - Open the database creates a slave. Become a new master as part of a replication tree. \n");
}
